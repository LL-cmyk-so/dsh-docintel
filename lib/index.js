...
export function resolveWithinWorkspace(input, workspaceRoot) {	const resolved = isAbsolute(input) ? resolve(input) : resolve(workspaceRoot, input);
...
function buildAddTool(store, config, workspaceRoot) {
	return {
		...
		async execute(args) {
			...
			if (!input) return { summary: "错误：缺少 path 参数", added: 0, updated: 0, skipped: 0, failed: 0, files: [] };
			try {
				const resolved = resolveWithinWorkspace(input, workspaceRoot);
				const info = await stat(resolved);
				const files = info.isDirectory() ? await collectFiles(resolved, !!args?.recursive) : [resolved];
				if (files.length === 0) {
					return { summary: `没有找到支持的文件类型（支持: .pdf .md .markdown .txt .text .csv .json）：${input}`, added: 0, updated: 0, skipped: 0, failed: 0, files: [] };
				}
				const lines = [];
				const fileOutcomes = [];
				let added = 0;
				...
				return { summary: `doc_add 失败：${error instanceof Error ? error.message : String(error)}`, added: 0, updated: 0, skipped: 0, failed: 0, files: [] };
			}
		},
		output: {
			schema: {
