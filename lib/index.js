...
/** Build the doc_delete tool: remove a document by id or file name. */
function buildDeleteTool(store) {
	return {
		name: "doc_delete",
		description: "从知识库删除已入库的文档（按 id 或文件名）。删除后该文档的检索块一并移除；若文件由面板上传，副本一并删除。删除不可恢复，删除前请确认。",
		parameters: {
			type: "object",
			properties: {
				id: {
					type: "number",
					description: "文档 id（用 doc_status 查看）"
				},
				name: {
					type: "string",
					description: "文档文件名（与 id 二选一；同名时删除第一个）"
				}
			}
		},
		async execute(args) {
			...
			let target = null;
			if (args?.id !== void 0 && args?.id !== null) {
				const id = Number(args.id);
				target = docs.find((d) => d.id === id) ?? null;
				if (!target) {
					return { ok: false, removed: false, message: `未找到 id=${args.id} 的文档。用 doc_status 查看当前列表。` };
				}
			} else if (typeof args?.name === "string" && args.name.trim() !== "") {
				const name = args.name.trim();
				target = docs.find((d) => d.name === name) ?? null;
				if (!target) {
					return { ok: false, removed: false, message: `未找到名为「${name}」的文档。用 doc_status 查看当前列表。` };
				}
			} else {
				return { ok: false, removed: false, message: "缺少参数：请提供 id 或 name（用 doc_status 查看当前列表）。" };
			}
			const result = store.removeDocument(target.id);
			if (result.removed && result.uploadsFile) {
				const { unlink } = await import("node:fs/promises");
				await unlink(result.uploadsFile).catch(() => { /* already gone */ });
			}
			return { ok: true, removed: result.removed, name: result.name ?? target.name, remaining: store.listDocuments().length };
		},
		output: {
			schema: {
				type: "object",
			...
			render: (_args, value) => [{ type: "text", text: value.message ?? (value.removed ? `已删除 ${value.name ?? ""}，剩余 ${value.remaining ?? 0} 个文档` : "未删除") }]
		}
	};
}
...
export function apply(ctx, config) {
	...
	try {
		...
		disposers.push(ctx.tools.register(buildDeleteTool(store)));
		ctx.logger?.debug?.("[dsh-docintel] four tools registered");
	} catch (error) {
