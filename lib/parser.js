 ...
 * DOCX: mammoth (pure JS) → markdown → the same heading→section parser.
 *
...
export async function parseFile(path, fileText) {
	...
	if (kind === "docx") {
		// mammoth → markdown → the same heading→section parser used for .md
		const result = await mammoth.convertToMarkdown({ path });
		const paras = parseMarkdown(result.value);
		return { kind, name, pages: null, chars: result.value.length, paras };
	}
	if (kind === "json") {
		let pretty = fileText;
		try {
