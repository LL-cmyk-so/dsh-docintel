...
- **中文友好检索**：FTS5 trigram（≥3 字词，BM25 排序）+ LIKE 兜底（≤2 字词如「万科」「茅台」）双通道
- **开放格式**：知识库就是普通 SQLite 文件，book-distiller / 财报插件等任何工具可直接 `SELECT` 读取（反哺底座）
...
```text
用户：把这份财报入库
agent：→ doc_add path=万科A2025年年度报告.PDF
      → doc_status
用户：万科 2024 年现金流情况如何？
agent：→ doc_search query=现金流
      → 「据《万科A2025年年度报告.PDF》第42页：经营现金流为负…」
```
...
│ [搜索：现金流、毛利率、万科……]      (搜索) │
│ 找到 3 条：                                │
│  ▸ 贵州茅台2025年财报.PDF · 第65页         │
│     …经营现金流为负，主要系…               │
│ ┌──────────────────────────────────────┐ │
│ │   拖拽文件到这里，或点击选择文件       │ │
│ └──────────────────────────────────────┘ │
│ 已入库 7 个文档：                          │
│  📄 贵州茅台2025年财报.PDF   143页  75块  │
└──────────────────────────────────────────┘
```
...
- **v0.1（已完成）**：入库 + 检索 + 状态，三个工具，页码级定位，中文双通道
- **v0.2（已完成）**：Web 管理面板（侧边栏入口 + 抽屉：搜索 / 拖拽上传 / 文档列表）
- **v0.3**：MinerU 高保真档（扫描件/复杂表格/公式）+ 可选本地向量混合检索
- **v0.4**：导出 Obsidian vault（复用 book-distiller 产出）+ 深度蒸馏模式 + 财报领域模板
